...
def triplet_loss(y_true, y_pred, N=4096, alpha=1.0):
    """
...
def get_embedding_cnn(in_dims, out_dims=4096):
    ...
    model.add(
        Conv2D(
            ...
            activation="relu",
            input_shape=(in_dims),
            kernel_initializer=RandomNormal(mean=0.0, stddev=0.01, seed=0),
    ...
    model.add(
        Conv2D(
            ...
            activation="relu",
            kernel_initializer=RandomNormal(mean=0.0, stddev=0.01, seed=0),
            bias_initializer=RandomNormal(mean=0.5, stddev=0.01, seed=0),
            kernel_regularizer=l2(2e-4),
        )
    )
    model.add(MaxPooling2D())

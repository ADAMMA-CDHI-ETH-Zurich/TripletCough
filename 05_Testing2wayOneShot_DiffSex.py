...
# 2-Way One-Shot Testing using only pairs of participants of opposite sex
...
with open(
    "%s%s%s%s%s%s%s%s%s"
    % (testing_path, timestamp, "train_", weights, "_test_DiffSex_", data_name, "_", n_tasks, "__testing.csv"),
    "w",
    newline="",
) as file:
    ...
    permulation_list = list(product(male_participants, female_participants)) + list(
        product(female_participants, male_participants)
    )
    ...
    for weight_file in os.listdir(weights_path):
        ...
        for participant_combo in list(product(male_participants, female_participants)) + list(
            product(female_participants, male_participants)
        ):
            n_test = 0
            n_correct = 0
